...
def welcome():
    ...
    print("Press Enter keyto Proceed")
    input()
def checkcsv():
    csv_files=[]
    cur_dir=os.getcwd()     #current directory is fetched
    content_list=os.listdir(cur_dir)
    for x in content_list:
        if x.split('.')[-1]=='csv':     #meaning of [-1] is last index of list of string
            csv_files.append(x)
    if len(csv_files)==0:
...
def main():
    ...
    try:
        ...
        print("\nEnter experiences in years of the candidates, saparated by comma")
